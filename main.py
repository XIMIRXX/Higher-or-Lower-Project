# Make sure art.py and game_data.py are in the same directory

from game_data import data
from art import logo, vs
import random

def account_info(account):
    name_account = account["name"]
    account_description = account["description"]
    account_country = account["country"]
    return f"{name_account}, a {account_description}, from {account_country}"

def check_answer(user_guess, a_followers, b_followers):
    if a_followers > b_followers:
        return user_guess == "a"
    else:
        return user_guess == "b"

def start_again():
    ask = input("Do you want to start the game again?(yes/no): ").lower()
    if ask == "yes":
        game_logic()
    elif ask == "no":
        return

def game_logic():
    print(logo)
    score = 0
    game_should_continue = True

    account_b = random.choice(data)

    while game_should_continue:
        account_a = account_b
        account_b = random.choice(data)

        if account_a == account_b:
            account_b = random.choice(data)

        print(f"Compare A: {account_info(account_a)}")
        print(vs)
        print(f"Compare B: {account_info(account_b)}")

        guess = input("Who has more followers?(a/b): ").lower()

        print("\n" * 20)
        print(logo)

        a_followers_count = account_a["follower_count"]
        b_followers_count = account_b["follower_count"]

        is_correct = check_answer(guess, a_followers_count, b_followers_count)

        if is_correct:
            score += 1
            print(f"You're right! Current score {score}")
        else:
            print(f"That's wrong. Final score: {score}.")
            game_should_continue = False
            start_again()

game_logic()
